...
resource "vault_kubernetes_auth_backend_role" "auth-read" {
  ...
  bound_service_account_namespaces = ["auth", "argocd", "vpn", "eclipse-che", "grafana", "registry", "exit-node-base", "monofolio", "guard", "chat", "dev-ws-max", "s3", "git-pages", "cert-manager", "ro", "weebo-runner"]
  token_ttl                        = 3600
